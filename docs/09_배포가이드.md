...
⚠️ **GitHub에서 누락되는 필수 파일들** (`.gitignore`로 인해 Git에 포함되지 않음):

- [ ] **ML 모델 파일** - `backend/flask_inference/final_model/` (용량 및 보안 문제)

✅ **배포 시 반드시 수행해야 할 작업**:

1. Git 저장소 클론
2. ML 모델 파일을 별도로 서버에 업로드 (섹션 3-1 참조)
3. `.env` 파일의 환경 변수 값을 실제 운영 환경에 맞게 수정 (섹션 3-2 참조)
4. `DEBUG=False` 설정 및 보안 설정 확인

> **💡 Tip**: `.env` 파일은 이제 Git에 포함되어 있으며 개발용 기본값이 설정되어 있습니다. 배포 시 반드시 실제 운영 환경 값으로 수정하세요.
...
NeuroNova는 두 가지 Nginx 설정 파일을 제공합니다:

1. **`neuronova.conf`** - 기본 설정 (빠른 배포용)
2. **`neuronova_optimized.conf`** - 최적화 설정 (프로덕션 권장)

#### 최적화 설정의 이점

**트래픽 절감:**
- gzip 압축: JavaScript/CSS 70-80% 크기 감소
- Brotli 압축: gzip보다 15-20% 추가 압축
- 정적 파일 캐싱: 반복 방문 시 대역폭 99% 절감

**성능 향상:**
- 브라우저 캐싱으로 로딩 속도 대폭 개선
- Open File Cache로 서버 I/O 부하 감소
- ETag로 변경되지 않은 파일 재전송 방지

**예상 효과:**
```
최초 방문: 5MB → 1.5MB (70% 감소)
재방문:   1.5MB → 50KB (97% 감소)
```
...
프로덕션 환경에서는 최적화된 설정을 사용하는 것을 강력히 권장합니다.

#### 최적화 설정 적용 방법

```bash
# 1. 최적화 설정 파일 복사
sudo cp ~/NeuroNova/deploy/nginx/neuronova_optimized.conf /etc/nginx/sites-available/neuronova

# 2. YOUR_DOMAIN_OR_IP를 실제 값으로 변경
sudo nano /etc/nginx/sites-available/neuronova

# 3. 심볼릭 링크 생성 (이미 있다면 건너뛰기)
...
gzip_types
    ...
    application/vnd.ms-fontobject image/svg+xml image/x-icon;
gzip_min_length 256;
gzip_disable "msie6";
```
...
**압축 확인:**
```bash
# HTTP 헤더에서 Content-Encoding 확인
curl -H 'Accept-Encoding: gzip' -I http://your-domain.com | grep -i content-encoding
# 출력: content-encoding: gzip
```

#### Brotli 압축 활성화 (선택사항, gzip보다 15-20% 더 압축)
...
# 출력: content-encoding: br
```

#### 이미지 최적화 (WebP 변환)

React 빌드 결과물의 이미지를 WebP로 변환하여 40-50% 추가 용량 절감:
...
스크립트가 다음 작업을 수행합니다:
- PNG/JPG 파일을 WebP로 변환 (품질 85%)
- 원본 파일은 유지 (폴백용)
- 변환 결과 및 절약된 용량 출력

**WebP 지원 확인:**
- 최적화된 Nginx 설정은 WebP를 지원하는 브라우저에 자동으로 WebP 제공
- 지원하지 않는 브라우저에는 원본 이미지 제공
...
| 파일 타입 | 캐시 기간 | 이유 |
|----------|----------|------|
| HTML | 캐시 안 함 | 항상 최신 버전 제공 |
| JS/CSS (해시) | 1년 | 파일명에 해시 포함으로 변경 시 자동 갱신 |
| 폰트 | 1년 | 거의 변경되지 않음 |
| 이미지 (일반) | 6개월 | 적당한 캐싱 기간 |
| DICOM | 캐시 안 함 | 민감한 의료 데이터, 항상 최신 |
| API 응답 | 캐시 안 함 | 동적 데이터 |
...
⚠️ **중요**: ML 모델 파일(`final_model`)은 용량이 크고 민감한 정보가 포함되어 있어 Git에 포함되지 않습니다.

#### 방법 1: SCP를 사용한 업로드 (권장)
...
#### 방법 2: WinSCP 또는 FileZilla 사용

1. WinSCP 또는 FileZilla를 실행합니다.
2. 서버에 SFTP로 접속합니다.
3. 로컬의 `backend/flask_inference/final_model` 폴더를 서버의 `~/NeuroNova/backend/flask_inference/` 경로로 드래그 앤 드롭합니다.
...
# Google Drive, AWS S3, 또는 다른 클라우드 스토리지에서 다운로드
# 예시 (wget 사용):
...
> **주의사항**:
> - `final_model` 디렉토리가 없으면 Flask 서버가 시작되지 않습니다.
> - 모델 파일은 보안이 중요하므로 안전한 방법으로 전송하세요.
> - 파일 크기가 크므로 네트워크 상태에 따라 시간이 걸릴 수 있습니다.

### 3-2) 환경 변수 파일 수정 (필수)

✅ `.env` 파일은 Git 저장소에 포함되어 있으며 개발용 기본값이 설정되어 있습니다.

⚠️ **중요**: 배포 시 반드시 실제 운영 환경 값으로 수정해야 합니다.
...
다음 항목들을 **반드시** 실제 값으로 변경하세요:

```bash
# Django Settings
SECRET_KEY=your-actual-secret-key-here  # ⚠️ 변경 필수!
DEBUG=False  # ⚠️ 배포 환경에서는 반드시 False
ALLOWED_HOSTS=your-domain.com,your-server-ip  # ⚠️ 실제 도메인/IP로 변경

# Database Configuration
DB_NAME=your-actual-database-name  # ⚠️ 실제 DB 이름
DB_USER=your-actual-database-user  # ⚠️ 실제 DB 사용자
DB_PASSWORD=your-actual-database-password  # ⚠️ 실제 DB 비밀번호
DB_HOST=your-actual-db-host  # ⚠️ 실제 DB 호스트

# Flask AI Inference Server
FLASK_INFERENCE_URL=http://127.0.0.1:9000  # 로컬 전용
FLASK_API_KEY=your-flask-api-key-here  # ⚠️ 강력한 API 키로 변경 (Flask와 동일하게!)

# JWT Settings
JWT_SECRET_KEY=your-actual-jwt-secret-key  # ⚠️ 변경 필수!
```
...
다음 항목을 Django와 일치하도록 수정하세요:

```bash
# API 키 (Django의 FLASK_API_KEY와 동일해야 함)
API_KEY=your-flask-api-key-here  # ⚠️ Django .env의 FLASK_API_KEY와 동일하게 설정
```

> **보안 주의사항**:
> - `SECRET_KEY`, `JWT_SECRET_KEY`: 최소 50자 이상의 무작위 문자열 사용
> - `DEBUG`: 배포 환경에서는 **반드시 False**
> - `ALLOWED_HOSTS`: 실제 도메인과 IP만 포함
> - **`FLASK_API_KEY` (Django) = `API_KEY` (Flask)**: 반드시 동일한 값으로 설정! 이 키로 Django-Flask 간 통신을 인증합니다.
> - Git에 민감한 정보를 커밋하지 않도록 주의 (`.env.production` 사용 권장)

#### ✅ 개선 사항 (2025-12-03 업데이트)

Django ml_proxy가 다음과 같이 개선되었습니다:

1. **환경 변수 사용**: 하드코딩된 Flask URL 대신 `.env` 파일의 `FLASK_INFERENCE_URL` 사용
2. **API 키 인증**: Django → Flask 요청 시 `X-API-Key` 헤더로 API 키 전달
3. **보안 강화**: Flask는 모든 요청에서 API 키 검증 수행

이제 Django ml_proxy는 `.env` 파일의 설정을 자동으로 읽어 Flask와 통신합니다.
...
# [선택사항] 테스트 데이터 생성 (주의: 개발/스테이징 환경에서만 사용)
...
> - `init_test_data` 명령어는 테스트 데이터를 생성합니다. `--clear` 옵션을 사용하면 기존 데이터가 모두 삭제되므로 주의하세요.
...
트래픽을 70-90% 절감하고 로딩 속도를 대폭 개선하려면 최적화를 적용하세요.
...
```bash
# 최적화 설정 파일 사용
sudo cp deploy/nginx/neuronova_optimized.conf /etc/nginx/sites-available/neuronova
...
sudo apt-get install -y libnginx-mod-http-brotli-filter libnginx-mod-http-brotli-static
sudo nano /etc/nginx/conf.d/brotli.conf
# (섹션 3-C의 Brotli 설정 참조)
```

### 9) 설정 파일 적용 및 서비스 재시작
```bash
# Nginx 설정 복사 (최초 1회)
# 기본 설정 사용 시:
...
# 압축 효과 측정
# 압축 전
curl -H 'Accept-Encoding: identity' http://your-domain.com -o /dev/null -s -w 'Size: %{size_download} bytes\n'
# 압축 후
curl -H 'Accept-Encoding: gzip' http://your-domain.com -o /dev/null -s -w 'Size: %{size_download} bytes\n'
```
...
2. 환경 변수 값이 기본값인지 확인:
   ```bash
   # Django .env에서 변경 필요한 항목 확인
   grep -E "SECRET_KEY|DEBUG|DB_PASSWORD|FLASK_API_KEY" ~/NeuroNova/backend/django_main/.env

   # Flask API 키가 Django와 일치하는지 확인
   grep "API_KEY" ~/NeuroNova/backend/flask_inference/.env
   ```
3. 기본값으로 설정된 경우 "3-2) 환경 변수 파일 수정" 섹션 참조

**ML 모델(final_model) 누락 오류:**
1. 모델 디렉토리 확인:
   ```bash
   ls -la ~/NeuroNova/backend/flask_inference/final_model/
   # 디렉토리가 없으면 "3-1) ML 모델 파일 업로드" 섹션 참조
   ```
2. Flask 서비스가 모델을 찾지 못하는 경우:
   ```bash
   # Flask 로그에서 모델 로딩 오류 확인
   sudo journalctl -u gunicorn_flask -n 100 | grep -i model

   # .env 파일의 MODEL_DIR 경로 확인
   cat ~/NeuroNova/backend/flask_inference/.env | grep MODEL_DIR
   ```

**Git에서 누락된 파일들:**

GitHub에서 클론 시 다음 파일은 포함되지 않습니다 (`.gitignore`에 명시됨):

- ❌ `backend/flask_inference/final_model/` (ML 모델 - 용량 및 보안 문제)
- ℹ️ `media/` (업로드된 파일 - 배포 과정에서 자동 생성)
- ℹ️ `staticfiles/` (빌드 결과물 - collectstatic으로 자동 생성)

✅ `.env` 파일은 이제 Git에 포함됨 (개발용 기본값 포함)

**해결 방법:**
1. **섹션 3-1**: `final_model` 디렉토리를 서버에 업로드
2. **섹션 3-2**: `.env` 파일의 값을 실제 운영 환경에 맞게 수정
3. `media/`와 `staticfiles/`는 배포 과정에서 자동으로 생성됨
