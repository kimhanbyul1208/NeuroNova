...
### 3) 시스템 패키지 설치 (최초 1회)
Django의 `mysqlclient` 설치를 위해 필요한 시스템 라이브러리를 설치합니다.
...
# MySQL/MariaDB 클라이언트 개발 라이브러리 설치
...
> **주의**:
> - 이 단계는 최초 서버 세팅 시 **1회만** 실행하면 됩니다.
> - `mysqlclient`는 C 확장 모듈이므로 컴파일을 위해 이러한 시스템 라이브러리가 필요합니다.
> - Python 버전이 다른 경우 (예: Python 3.10), `python3.10-dev`로 지정할 수 있습니다.
...
> - `mysqlclient` 설치가 실패하는 경우, 위의 **3) 시스템 패키지 설치** 단계를 먼저 수행했는지 확인하세요.

### 7) AI Server (Flask) 설정
```bash
